...
public class TransparentPersistenceExample extends Util {
	...
	public static void storeCarAndSnapshots() {
		...
		transactions.Car car = new transactions.Car("Ferrari");
		for (int i = 0; i < 3; i++) {
			car.snapshot();
		}
		db.store(car);
	...
	public static void modifySnapshotHistory() {
		...
		ObjectSet result = db.queryByExample(transactions.Car.class);
		if (result.hasNext()) {
			transactions.Car car = (transactions.Car) result.next();
			transactions.SensorReadout readout = car.getHistory();
			while (readout != null) {
